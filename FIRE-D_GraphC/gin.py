...
class Encoder(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        #print("hellox:", x.size()) # num_nodes in batch \times hidden_dim
        if x is None:
        ...
        for i in range(self.num_gc_layers):
            ...
            x = self.bns[i](x)
            xs.append(x)
            #print("afterx:", x.size()) # torch.Size([113, 32])

        xpool = [global_add_pool(x, batch) for x in xs]
        #print("xpool:", len(xpool), xpool[0].size()) # xpool: 5
        x = torch.cat(xpool, 1)
        #print("x (after):", x.size()) # x (after): torch.Size([8, 160])
        #print("torch.cat(xs, 1) is:", torch.cat(xs, 1).size()) # torch.Size([113, 160])
    ...
    def get_embeddings(self, loader):
        ...
        with torch.no_grad():
            for data in loader:
                data.to(device)
                x, edge_index, batch = data.x, data.edge_index, data.batch # 按batch
                if x is None:
                    x = torch.ones((batch.shape[0],1)).to(device)
